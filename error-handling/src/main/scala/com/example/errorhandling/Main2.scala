    ...
    def dsl[F[_]](
      ...
      new ErrorHandler[F] {
        ...
        override def badTechnicalProgram: F[Unit] =
          F.handleErrorWith(F.flatMap(dsl.badTechnical)(console.good)) {
            case NonFatal(throwable) => console.bad(throwable)
          }

        override def goodBusinessProgram: F[Unit] =
          G.handleErrorWith(F.flatMap(dsl.goodBusiness)(console.good)) {
            case businessError => console.bad(businessError)
          }

        override def badBusinessProgram: F[Unit] =
          G.handleErrorWith(F.flatMap(dsl.badBusiness)(console.good)) {
            case businessError => console.bad(businessError)
          }
      }
